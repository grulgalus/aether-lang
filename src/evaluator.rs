...
pub fn eval_program(program: &Program, env: &mut Environment) -> Object {
    ...
    for stmt in &program.statements {
        ...
        if let Object::Error(e) = result { return Object::Error(e); }
    }
    result
}
    ...
    match expr {
        ...
        Expr::Call { function, args } => {
            ...
            match function.as_str() {
                ...
                // 🌐 TADY JE NOVÁ MAGIE: AETHER WEB SERVER!
                "serve" => {
                    if let (Some(Object::Number(port)), Some(Object::StringObj(html))) = (eval_args.get(0), eval_args.get(1)) {
                        let addr = format!("0.0.0.1:{}", *port as u16);
                        println!("🌐 Aether Server běží na http://127.0.0.1:{}", *port as u16);
                        ...
                        } else { return Object::Error(format!("Port {} je už obsazený!", port)); }
                        return Object::Null;
                    }
