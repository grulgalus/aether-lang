...
fn eval_block(statements: &[Stmt], env: &mut Environment) -> Object { let mut result = Object::Null; for stmt in statements { result = eval_statement(stmt, env); if let Stmt::Return { .. } = stmt { break; } } result }
fn eval_statement(stmt: &Stmt, env: &mut Environment) -> Object {
    match stmt {
        ...
        Stmt::Print { value } => { let text = eval_expression(value, env).to_string_val(); println!("{}", text); env.output.push(text); Object::Null }
    ...
    match expr {
        ...
        Expr::Call { function, args } => {
            ...
            match function.as_str() {
                "env" => { if let Some(Object::StringObj(k)) = eval_args.get(0) { return Object::StringObj(std::env::var(k).unwrap_or_else(|_| "".to_string())); } Object::Null },
                "load_env" => { if let Some(Object::StringObj(path)) = eval_args.get(0) { let mut map = HashMap::new(); if let Ok(content) = std::fs::read_to_string(path) { for line in content.lines() { let l = line.trim(); if l.is_empty() || l.starts_with('#') { continue; } if let Some((k, v)) = l.split_once('=') { let val = v.trim().trim_matches('"').trim_matches('\''); map.insert(k.trim().to_string(), Object::StringObj(val.to_string())); } } } return Object::Dict(map); } Object::Null },
                "parse_json" => { if let Some(Object::StringObj(content)) = eval_args.get(0) { let lexer = Lexer::new(&content); let mut parser = Parser::new(lexer); let program = parser.parse_program(); if let Some(Stmt::Expression(expr)) = program.statements.first() { return eval_expression(expr, env); } } Object::Null },
                ...
                // --- NOVÉ FUNKCE PRO ZPRACOVÁNÍ TEXTU A ČÍSEL ---
                "upper" => { if let Some(Object::StringObj(s)) = eval_args.get(0) { return Object::StringObj(s.to_uppercase()); } Object::Null },
                "lower" => { if let Some(Object::StringObj(s)) = eval_args.get(0) { return Object::StringObj(s.to_lowercase()); } Object::Null },
                "split" => { if let (Some(Object::StringObj(s)), Some(Object::StringObj(d))) = (eval_args.get(0), eval_args.get(1)) { return Object::Array(s.split(d.as_str()).map(|x| Object::StringObj(x.to_string())).collect()); } Object::Null },
                "str" => { if let Some(obj) = eval_args.get(0) { return Object::StringObj(obj.to_string_val()); } Object::Null },
                "int" => { if let Some(obj) = eval_args.get(0) { match obj { Object::StringObj(s) => return Object::Number(s.parse().unwrap_or(0.0)), Object::Number(n) => return Object::Number(*n), Object::Boolean(b) => return Object::Number(if *b { 1.0 } else { 0.0 }), _ => return Object::Number(0.0) } } Object::Null },
                _ => Object::Null
            }
        }
