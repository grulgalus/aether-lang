...
#[allow(dead_code)]
fn optimize_expr(expr: Expr) -> Expr { expr }
    ...
    match stmt {
        ...
        Stmt::While { condition, body } => { 
            ...
            loop { 
                ...
                let is_truthy = match cond { Object::Boolean(b) => b, Object::Number(n) => n != 0.0, Object::Null => false, _ => true }; 
                if !is_truthy { break; } 
                res = eval_block(body, env); if let Object::Error(_) = res { return res; }
            } 
            res 
        }
...
fn eval_expression(expr: &Expr, env: &Environment) -> Object {
    match expr {
        ...
        Expr::Index { left, index } => { 
            ...
            let i = eval_expression(index, env); if let Object::Error(_) = i { return i; }
            if let (Object::Array(arr), Object::Number(idx)) = (&l, &i) { let index_usize = *idx as usize; if index_usize < arr.len() { return arr[index_usize].clone(); } else { return Object::Error("Index je mimo pole".into()); } }
            if let (Object::Dict(map), Object::StringObj(k)) = (&l, &i) { if let Some(val) = map.get(k) { return val.clone(); } else { return Object::Error("Klíč neexistuje".into()); } }
            Object::Error("Lze indexovat jen pole a dict".into())
        }
        ...
        Expr::BinaryOp { left, operator, right } => {
            ...
            if let (Object::Number(v1), Object::Number(v2)) = (&l, &r) {
                match operator.as_str() {
                    "+" => Object::Number(v1 + v2), "-" => Object::Number(v1 - v2), "*" => Object::Number(v1 * v2), "/" => if *v2 != 0.0 { Object::Number(v1 / v2) } else { Object::Error("Dělení nulou".into()) },
                    "<" => Object::Boolean(v1 < v2), ">" => Object::Boolean(v1 > v2), "==" => Object::Boolean(v1 == v2), "!=" => Object::Boolean(v1 != v2), "<=" => Object::Boolean(v1 <= v2), ">=" => Object::Boolean(v1 >= v2),
                    _ => Object::Error("Neznámý operátor".into())
                }
            } else if let (Object::StringObj(s1), Object::StringObj(s2)) = (&l, &r) {
