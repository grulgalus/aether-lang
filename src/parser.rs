...
impl Parser {
    ...
    fn parse_block(&mut self) -> Vec<Stmt> {
        ...
        self.next_token(); // přeskočí '{' nebo jiný token
    ...
    fn parse_return(&mut self) -> Option<Stmt> {
        self.next_token(); let value = self.parse_expression()?; self.next_token(); Some(Stmt::Return { value })
    }
