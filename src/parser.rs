...
impl Parser {
    ...
    fn parse_statement(&mut self) -> Option<Stmt> {
        match &self.current_token {
            ...
            Token::Keyword(kw) if kw == "for" => self.parse_for(),
            Token::Keyword(kw) if kw == "import" => { self.next_token(); let file = match &self.current_token { Token::StringLiteral(s) => s.clone(), _ => return None }; self.next_token(); Some(Stmt::Import(file)) }
            Token::Identifier(name) => {
                if let Token::Operator(op) = &self.peek_token { if op == "=" { let n = name.clone(); self.next_token(); self.next_token(); let value = self.parse_expression()?; self.next_token(); return Some(Stmt::Assign { name: n, value }); } }
                let expr = self.parse_expression()?; self.next_token(); Some(Stmt::Expression(expr))
            }
            _ => None,
        }
    }
