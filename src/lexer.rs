...
impl Lexer {
    ...
    fn read_string(&mut self) -> Token {
        ...
        let text: String = self.input[start..self.position].iter().collect();
        self.position += 1; Token::StringLiteral(text)
    }
    fn read_number(&mut self) -> Token {
        let start = self.position;
        while self.position < self.input.len() && (self.input[self.position].is_ascii_digit() || self.input[self.position] == '.') { self.position += 1; }
